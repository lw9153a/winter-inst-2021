...
A beta regression was also considered when creating our model. The nature of our response variable, opinion on the need for presidential and congressional action on global warming, led us to consider a beta regression. After comparing the actual and predicted values, the RMSE shows us that both models were predicting almost identical values. Since the linear regression is simpler to interpret and does not sacrifice accuracy, it was chosen as our final model over the beta regression. 
